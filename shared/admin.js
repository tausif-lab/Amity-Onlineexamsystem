...
window.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('openLiveProctoring').addEventListener('click', function() {
        // Get fresh user data from localStorage
        const currentUser = JSON.parse(localStorage.getItem('user') || '{}');
        ...
        if (currentUser.collegeId && currentUser.user1Id && currentUser.branch) {
            // University user
            window.location.href = `/adminlive.html?collegeId=${encodeURIComponent(currentUser.collegeId)}&userId=${encodeURIComponent(currentUser.user1Id)}&branch=${encodeURIComponent(currentUser.branch)}`;
        } else {
            // Coaching user
            window.location.href = `/adminlive.html?Email=${encodeURIComponent(currentUser.email)}&id=${encodeURIComponent(currentUser.id || currentUser._id)}`;
        }
    });
});
